...
public class CardScout extends Card {
    public CardScout(String name, int squad, CardGroup cardGroup) {
        this.name = name;
        this.squad = squad;
        this.initiative = 6;
    ...
    public void pickAction(Bot bot) {
        ...
        if (bot == null) {
            ...
            if (suppressed) {
                ...
                System.out.println("2 to scout(2),");
                System.out.println("3 to attack(1),");
                System.out.println("4 to recon,");
                System.out.println("5 to conceal");
            }
            ...
            switch (choice) {
                ...
                case (2):
                    ...
                    if (!suppressed) success = this.scout(2,token,bot);
                ...
                case (4):
                    if (!tokenExists) token = this.placeToken();
                    if (!suppressed) success = this.recon();
                    if (!suppressed && success) this.playCard();
                    if (!tokenExists && !success) token.deletDis();
                    break;
                case (5):
                    if (!tokenExists) token = this.placeToken();
                    if (!suppressed) success = this.conceal();
                    if (!suppressed && success) this.playCard();
                    if (!tokenExists && !success) token.deletDis();
                    break;
            }
            ...
            boolean willScout = Integer.parseInt(choices.substring(38,39)) == 1;
            boolean willAttack = Integer.parseInt(choices.substring(40,41)) == 1;
            boolean willRecon = Integer.parseInt(choices.substring(45,46)) == 1;
            boolean willConceal = Integer.parseInt(choices.substring(46,47)) == 1;
            ...
            if ( willScout || willAttack || willRecon || willConceal ) {
                if ( suppressed ) {
                ...
                if (willScout) {
                    success = this.scout(2,token,bot);
                    System.out.println( bot.getName() + ": tried scouting" );
                }
                else if (willAttack) {
                    ...
                    success = this.recon();
                    System.out.println( bot.getName() + ": tried reconing" );
                }
                else {
                    success = this.conceal();
                    System.out.println( bot.getName() + ": tried concealing" );
                }
